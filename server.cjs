...
app.post("/api/proxy", async (req, res) => {
  ...
  res.json = function(body) {
    if (body && typeof body === "object") {
      if (body.status === "success" && body.success === void 0) {
        body.success = true;
      } else if (body.status === "error" && body.success === void 0) {
        body.success = false;
      } else if (body.success === true && body.status === void 0) {
        body.status = "success";
      } else if (body.success === false && body.status === void 0) {
        body.status = "error";
      }
    }
  ...
  try {
    ...
    if (action === "login") {
      ...
      const matchedUser = users.find((u) => String(u.username || "").trim().toLowerCase() === cleanUsername);
      if (matchedUser) {
    ...
    if (action === "addUser") {
      ...
      const users = db.users || [];
      if (users.some((u) => String(u.username || "").trim().toLowerCase() === user.toLowerCase())) {
        return res.json({ status: "error", message: "Username sudah digunakan." });
      }
      if (payload.uid && users.some((u) => String(u.uid).trim() === String(payload.uid).trim())) {
    ...
    if (action === "updateUser") {
      ...
      const idx = users.findIndex((u) => String(u.username || "").trim().toLowerCase() === String(oldUsername || "").trim().toLowerCase());
      if (idx !== -1) {
        ...
        users[idx] = {
          ...users[idx],
          name: payload.name !== void 0 ? payload.name : users[idx].name,
          username: payload.username !== void 0 ? payload.username : users[idx].username,
          password: payload.password ? payload.password : users[idx].password,
          uid: payload.uid !== void 0 ? payload.uid : users[idx].uid,
          role: payload.role !== void 0 ? payload.role : users[idx].role,
          status: payload.status !== void 0 ? payload.status : users[idx].status,
          photoUrl: payload.photoBase64 ? finalPhotoUrl : payload.photoUrl !== void 0 ? payload.photoUrl : users[idx].photoUrl
        };
        db.users = users;
        saveDb(db);
    ...
    if (action === "deleteUser") {
      const users = db.users || [];
      const idx = users.findIndex((u) => String(u.username || "").trim().toLowerCase() === String(payload.username || "").trim().toLowerCase());
      if (idx !== -1) {
        users.splice(idx, 1);
        db.users = users;
        saveDb(db);
    ...
    if (action === "getDailyData") {
      ...
      if (payload.role === "Staff") {
        ...
        rows = rows.filter(
          (r) => String(r.recruiter || "").toLowerCase() === usernameLower || String(r.recruiter || "").toLowerCase() === nameLower
        );
      }

...
export const DailyData = ({ authUser }) => {
    ...
    const isPrivileged = authUser && (authUser?.role === 'Superadmin' || authUser?.role === 'Admin');
    ...
    const handleOpenAdd = () => {
        if (!isPrivileged) {
            showToastMessage('error', 'Hanya Admin dan Superadmin yang diizinkan untuk menginput data.');
            return;
        }
        setModalMode('add');
    ...
    const handleOpenEdit = (item) => { 
        if (!item) return;
        if (!isPrivileged) {
            showToastMessage('error', 'Hanya Admin dan Superadmin yang diizinkan untuk mengubah data.');
            return;
        }
        setModalMode('edit'); 
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isPrivileged) {
            showToastMessage('error', 'Hanya Admin dan Superadmin yang diizinkan untuk menginput atau mengubah data.');
            return;
        }
    ...
    const confirmDelete = async () => {
        ...
        if (!isPrivileged) {
            showToastMessage('error', 'Hanya Admin dan Superadmin yang diizinkan untuk menghapus data.');
            return;
        }
    ...
    const statCards = [
        { l: 'Total', v: statTotal, bg: 'bg-blue-50 dark:bg-blue-500/10', text: 'text-blue-500', txtColor: 'text-blue-600 dark:text-blue-400', i: 'ph-users' },

...
export const DailyStats = ({ authUser }) => {
    ...
    const [pageKurangTarget, setPageKurangTarget] = useState(1);
    const [pageIzin, setPageIzin] = useState(1);
    const [pageBelumLapor, setPageBelumLapor] = useState(1);

    useEffect(() => {
        setPageSesuaiTarget(1);
        setPageKurangTarget(1);
        setPageIzin(1);
        setPageBelumLapor(1);
    }, [harianDate]);
    ...
    const [formData, setFormData] = useState<any>({ 
        id: '', tanggalKerja: '', tanggalLapor: '', username: '', postingan: '', kunjungan: '', pelamar: '', pengujian: '', status: 'Draft', isIzin: false, keteranganIzin: '' 
    });
    ...
    const getDeadlineForWorkDate = (workDateStr) => {
        if (!workDateStr) return new Date();
        ...
        return new Date(`${nextYear}-${nextMonth}-${nextDay}T10:00:00+07:00`);
    };

    const getReportStatus = (p, tanggalKerja) => {
        const baseStatus = p ? (p.status || 'Submitted') : 'Draft';
        if (baseStatus === 'Reopened') {
            return 'Reopened';
        }
        if (baseStatus === 'Locked') {
            return 'Locked';
        }
        const deadline = getDeadlineForWorkDate(tanggalKerja);
    ...
    const getCountdownStr = (workDateStr) => {
        const deadline = getDeadlineForWorkDate(workDateStr);
    ...
    const getStatusBadgeClass = (status) => {
        switch (status) {
            case 'Locked':
                return 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800/50';
            case 'Submitted':
    ...
    const getCandidatesCountForDate = (dateStr) => {
        return dailyCandidates.filter(c => {
            if (!c.tanggal) return false;
            if (c.results !== 'Acc') return false;
            let dStr = "";
            try {
                const d = new Date(c.tanggal);
                if (!isNaN(d.getTime())) dStr = new Date(d.getTime() - (d.getTimezoneOffset() * 60000)).toISOString().split('T')[0];
            } catch(e){}
            return dStr === dateStr && (!isPrivileged ? c.recruiter === authUser?.username : true);
        }).length;
    };

    const handleReopen = (p) => {
        if (!window.confirm(`Buka kembali laporan untuk ${p.username}?`)) return;
        const updatedPayload = { ...p, status: 'Reopened' };
        let newData = perfData.map(item => item.id === p.id ? updatedPayload : item);
        setPerfData(newData);
        localStorage.setItem('recruitOps_perfData', JSON.stringify(newData));
        fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'addPerfData', ...updatedPayload }) }).catch(()=>{});
    };

    const handleLock = (p) => {
        if (!window.confirm(`Kunci laporan untuk ${p.username}?`)) return;
        const updatedPayload = { ...p, status: 'Locked' };
        let newData = perfData.map(item => item.id === p.id ? updatedPayload : item);
        setPerfData(newData);
        localStorage.setItem('recruitOps_perfData', JSON.stringify(newData));
        fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'addPerfData', ...updatedPayload }) }).catch(()=>{});
    ...
    const evaluateH1 = (tanggalKerja, tanggalLapor, totalHarian, postingan, status, isIzin = false) => {
        ...
        if (isIzin) {
            return { reqPosting: 0, isMet: true, denda: 0, statusLapor: 'Izin', statusTarget: 'Izin (Bebas Denda)', dendaTelat: 0 };
        }

        if (!tanggalLapor || status === 'Draft') {
            if (now > deadline) { statusLapor = 'Belum Lapor (Telat)'; dendaTelat = 5000; } 
            ...
            if (tl > deadlineDay) { statusLapor = 'Telat Lapor'; dendaTelat = 5000; } 
        ...
        if (Number(postingan || 0) < reqPosting) {
            ...
            if ((tanggalLapor && status !== 'Draft') || now > deadline) dendaTarget = 5000;
        }
    ...
    const generateWeeklySummary = (offset, listToProcess) => {
        ...
        return listToProcess.map(staff => {
            ...
            let stats = { username, name: staff.name, posts:0, kunjungan:0, pelamar:0, pengujian:0, t0:0, v0:0, totalHarian:0, missCount:0, denda:0, dapatBonus: false };
            let evaluatedDays = 0;
            ...
            weekDays.forEach(dateStr => {
                ...
                const status = p ? getReportStatus(p, dateStr) : 'Draft';
                const isIzin = p?.isIzin || p?.status === 'Izin' || p?.isIzin === 'true';
                
                const evalDay = evaluateH1(dateStr, p ? p.tanggalLapor : null, t0v0.totalHarian, posts, status, isIzin);
    ...
    const evalForm = evaluateH1(formData.tanggalKerja, formData.tanggalLapor || new Date().toISOString().split('T')[0], activeFormStats.totalHarian, formData.postingan, formData.status || 'Draft', formData.isIzin || formData.status === 'Izin' || formData.isIzin === 'true');

    const handleSavePerf = (e) => {
        e.preventDefault();
        const statusValue = formData.status || 'Submitted';
        const payload = { 
            ...
            postingan: formData.isIzin ? 0 : (parseInt(formData.postingan) || 0), 
            kunjungan: formData.isIzin ? 0 : (parseInt(formData.kunjungan) || 0), 
            pelamar: formData.isIzin ? 0 : (parseInt(formData.pelamar) || 0), 
            pengujian: formData.isIzin ? 0 : (parseInt(formData.pengujian) || 0),
            status: statusValue,
            isIzin: formData.isIzin || false,
            keteranganIzin: formData.keteranganIzin || ''
        };
    ...
    const displayHarian = viewableStaffList.map(staff => {
        ...
        const status = p ? getReportStatus(p, harianDate) : 'Draft';
        const isIzin = p?.isIzin || p?.status === 'Izin' || p?.isIzin === 'true';
        
        let evalDay = evaluateH1(harianDate, p ? p.tanggalLapor : null, t0v0.totalHarian, posts, status, isIzin);
        let statusHarian = '';
        ...
        if (isBeforeJoin) {
            ...
            if (isIzin) {
                statusHarian = 'Izin';
            } else if (!hasInput && evalDay.dendaTelat === 0) {
                statusHarian = 'Belum Lapor';
            } else if (evalDay.denda === 0) {
                statusHarian = 'Sesuai Target';
            } else {
                statusHarian = 'Kurang Target';
            }
        }

        return { username, name: staff.name, posts, kunjungan: p && p.kunjungan ? Number(p.kunjungan) : 0, pelamar: p && p.pelamar ? Number(p.pelamar) : 0, pengujian: p && p.pengujian ? Number(p.pengujian) : 0, ...t0v0, denda: evalDay.denda, statusHarian, reqPosting: evalDay.reqPosting, telat: evalDay.statusLapor, isIzin, keteranganIzin: p?.keteranganIzin || '' };
    }).filter(s => s.statusHarian !== 'Belum Bergabung'); 

    const listSesuaiTarget = displayHarian.filter(s => s.statusHarian === 'Sesuai Target');
    const paginatedSesuaiTarget = listSesuaiTarget.slice((pageSesuaiTarget - 1) * 5, pageSesuaiTarget * 5);

    const listKurangTarget = displayHarian.filter(s => s.statusHarian === 'Kurang Target');
    const paginatedKurangTarget = listKurangTarget.slice((pageKurangTarget - 1) * 5, pageKurangTarget * 5);

    const listIzin = displayHarian.filter(s => s.statusHarian === 'Izin');
    const paginatedIzin = listIzin.slice((pageIzin - 1) * 5, pageIzin * 5);

    const listBelumLapor = displayHarian.filter(s => s.statusHarian === 'Belum Lapor');
    const paginatedBelumLapor = listBelumLapor.slice((pageBelumLapor - 1) * 5, pageBelumLapor * 5);
        ...
        return (
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100 dark:border-gray-700/50">
                <button 
                    ...
                    className="p-1 text-gray-400 dark:text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:hover:text-gray-400 transition-colors"
                >
    ...
    const renderHarianCard = (s) => (
        <div key={s.username} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm flex flex-col gap-4 hover:shadow-md transition-all group">
            <div className="flex justify-between items-start">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <div className="font-black text-sm text-gray-900 dark:text-white truncate" title={s.name || s.username}>{s.name || s.username}</div>
                ...
                <div className="shrink-0 ml-2">
                    ...
                    {s.statusHarian === 'Izin' && <span className="bg-amber-50 text-amber-600 border border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50 px-2.5 py-1 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center"><i className="ph-bold ph-hand-palm mr-1 text-amber-500"></i> Izin</span>}
            ...
            {s.statusHarian === 'Izin' ? (
                <div className="bg-amber-50/50 dark:bg-amber-950/20 p-3 rounded-xl border border-amber-100 dark:border-amber-900/30 text-xs text-amber-800 dark:text-amber-400">
                    <div className="font-black uppercase text-[9px] text-amber-600 dark:text-amber-300 tracking-wider mb-1"><i className="ph-fill ph-info mr-1"></i> Keterangan Izin:</div>
                    <p className="font-semibold italic leading-relaxed">{s.keteranganIzin || 'Izin Ga Kena Denda'}</p>
                </div>
            ) : (
        ...
        return (
            <tr key={username} className={`hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors group ${evalP.dendaTelat > 0 && !p ? 'bg-rose-50/30' : ''} ${isSelesai ? 'opacity-80 hover:opacity-100 bg-gray-50/50 dark:bg-gray-900/20' : ''}`}>
                <td className="px-5 py-4">
                    <div className="font-black text-sm text-gray-900 dark:text-white flex items-center">
                        <i className="ph-fill ph-user-circle mr-2 text-gray-400 text-lg"></i>
                        {staff.name}
                    </div>
                    <div className="text-[10px] text-gray-500 mt-1 font-bold bg-gray-100 dark:bg-gray-800 w-max px-2 py-0.5 rounded border border-gray-200 dark:border-gray-700 flex items-center gap-1.5">
                        <i className="ph-bold ph-at"></i>{username}
                        <span className={`px-1.5 py-0.2 rounded text-[8px] font-black uppercase border ${getStatusBadgeClass(status)}`}>
                            {status}
                        </span>
                    </div>
                </td>
                ...
                <td className="px-5 py-4 text-center">
                    ...
                    {evalP.dendaTelat > 0 && !p ? <span className="bg-rose-100 text-rose-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">Telat Lapor</span> : 
                     p ? (evalP.denda > 0 ? <span className="bg-rose-100 text-rose-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">Denda {evalP.denda/1000}K</span> : <span className="bg-emerald-100 text-emerald-600 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm">Aman</span>) :
                ...
                <td className="px-5 py-4 text-right">
                    {isLocked ? (
                        <div className="inline-flex items-center text-gray-400 bg-gray-100 dark:bg-gray-800 px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-[10px] font-bold uppercase tracking-wider">
                            <i className="ph-bold ph-lock mr-1 text-sm"></i> Locked
                        </div>
                    ) : p ? (
                        <div className="flex justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            <button onClick={()=>{setModalMode('edit'); setFormData(p); setIsModalOpen(true);}} className="w-8 h-8 flex items-center justify-center text-gray-500 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg shadow-sm transition-all" title="Edit"><i className="ph-bold ph-pencil-simple"></i></button>
                            {isPrivileged && (
                                <>
                                    {status === 'Locked' ? (
                                        <button onClick={()=>handleReopen(p)} className="w-8 h-8 flex items-center justify-center text-amber-500 bg-white dark:bg-gray-800 border border-amber-200 dark:border-gray-600 hover:text-amber-600 hover:border-amber-300 hover:bg-amber-50 rounded-lg shadow-sm transition-all" title="Reopen / Unlock"><i className="ph-bold ph-lock-key-open"></i></button>
                                    ) : (
                                        <button onClick={()=>handleLock(p)} className="w-8 h-8 flex items-center justify-center text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 hover:text-rose-500 hover:border-rose-300 hover:bg-rose-50 rounded-lg shadow-sm transition-all" title="Lock Report"><i className="ph-bold ph-lock"></i></button>
                                    )}
                                    <button onClick={()=>handleDelete(p.id)} className="w-8 h-8 flex items-center justify-center text-gray-500 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 hover:text-rose-600 hover:border-rose-300 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg shadow-sm transition-all" title="Hapus"><i className="ph-bold ph-trash"></i></button>
                                </>
                            )}
                        </div>
                    ) : (
                        <div className="flex justify-end gap-2">
                            <button onClick={()=>{setModalMode('add'); setFormData({ id: '', tanggalKerja: filterTanggalKerja, tanggalLapor: '', username: username, postingan: '', kunjungan: '', pelamar: '', pengujian: '', status: 'Submitted', isIzin: !isPrivileged, keteranganIzin: '' }); setIsModalOpen(true);}} className="px-4 py-2 bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-indigo-700 shadow-sm transition">Input Laporan</button>
                        </div>
                    )}
                </td>
            </tr>
            ...
            <div key={username} className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm relative ${isSelesai ? 'opacity-80 border-dashed bg-gray-50/50 dark:bg-gray-900/20' : ''}`}>
                ...
                <div className="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <div className="text-[10px] text-gray-500 mt-1 font-bold bg-gray-100 dark:bg-gray-700 w-max px-2 py-0.5 rounded border border-gray-200 dark:border-gray-600 flex items-center gap-1.5">
                            {username}
                            <span className={`px-1.5 py-0.2 rounded text-[8px] font-black uppercase border ${getStatusBadgeClass(status)}`}>
                                {status}
                            </span>
                    ...
                    {isLocked ? (
                        <div className="text-gray-400 bg-gray-100 dark:bg-gray-800 px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-[9px] font-black uppercase tracking-wider flex items-center">
                            <i className="ph-bold ph-lock mr-1"></i> Locked
                        </div>
                    ) : p ? (
                        <div className="flex gap-2">
                            ...
                            {isPrivileged && (
                                <>
                                    {status === 'Locked' ? (
                                        <button onClick={()=>handleReopen(p)} className="w-8 h-8 flex items-center justify-center text-amber-500 bg-amber-50 dark:bg-amber-900/20 rounded-lg"><i className="ph-bold ph-lock-key-open text-lg"></i></button>
                                    ) : (
                                        <button onClick={()=>handleLock(p)} className="w-8 h-8 flex items-center justify-center text-gray-400 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700"><i className="ph-bold ph-lock text-lg"></i></button>
                                    )}
                ...
                <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-700/50 pt-3">
                    ...
                    {!p ? (
                        <button onClick={()=>{setModalMode('add'); setFormData({ id: '', tanggalKerja: filterTanggalKerja, tanggalLapor: '', username: username, postingan: '', kunjungan: '', pelamar: '', pengujian: '', status: 'Submitted', isIzin: !isPrivileged, keteranganIzin: '' }); setIsModalOpen(true);}} className="px-3 py-1.5 bg-indigo-600 text-white font-black text-[10px] uppercase rounded-lg shadow-sm">Input</button>
                    ) : (
                        <div className="text-right">
                            <div className="text-xs font-black text-indigo-600 dark:text-indigo-400">{auto.totalHarian} Leads</div>
                            {evalP.denda > 0 ? <span className="text-[9px] font-black text-rose-600">Denda {evalP.denda/1000}K</span> : <span className="text-[9px] font-black text-emerald-600">Aman</span>}
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-gray-800 min-h-[400px]">
                    ...
                    {activeTab === 'input' && (() => {
                        const inputItems = viewableStaffList.map(staff => {
                            ...
                            const statusVal = getReportStatus(p, filterTanggalKerja);
                            const evalP = evaluateH1(filterTanggalKerja, p ? p.tanggalLapor : null, auto.totalHarian, p ? p.postingan : 0, statusVal, p?.isIzin || p?.status === 'Izin' || p?.isIzin === 'true');
                        ...
                        const myP = perfData.find(x => x.username === authUser?.username && x.tanggalKerja === filterTanggalKerja);
                        const reportStatus = getReportStatus(myP, filterTanggalKerja);
                        const deadlineDate = getDeadlineForWorkDate(filterTanggalKerja);
                        const countdownText = getCountdownStr(filterTanggalKerja);
                        const totalCandidates = getCandidatesCountForDate(filterTanggalKerja);

                        return (
                            <div className="space-y-6">
                                {/* BENTO GRID PANEL */}
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-5 pb-0">
                                    {/* CARD 1: TANGGAL KERJA & DEADLINE */}
                                    <div className="bg-gradient-to-br from-indigo-50 to-indigo-100/50 dark:from-indigo-950/20 dark:to-gray-800 border border-indigo-100 dark:border-indigo-800/60 p-4 rounded-2xl shadow-sm flex items-center gap-4">
                                        <div className="p-3 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-xl">
                                            <i className="ph-fill ph-calendar-blank text-2xl"></i>
                                        </div>
                                        <div>
                                            <div className="text-[10px] uppercase font-black tracking-widest text-indigo-500 mb-0.5">Tanggal Kerja</div>
                                            <div className="font-black text-sm text-gray-900 dark:text-white">{formatToDDMMYYYY(filterTanggalKerja)}</div>
                                            <div className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mt-1">Deadline: {deadlineDate.toLocaleDateString('id-ID', { weekday: 'long' })}, {formatToDDMMYYYY(deadlineDate.toISOString().split('T')[0])} 10.00 WIB</div>
                                        </div>
                                    </div>

                                    {/* CARD 2: COUNTDOWN */}
                                    <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/20 dark:to-gray-800 border border-amber-100 dark:border-amber-800/60 p-4 rounded-2xl shadow-sm flex items-center gap-4">
                                        <div className="p-3 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-xl">
                                            <i className="ph-fill ph-timer text-2xl"></i>
                                        </div>
                                        <div>
                                            <div className="text-[10px] uppercase font-black tracking-widest text-amber-500 mb-0.5">Countdown Deadline</div>
                                            <div className={`font-black text-sm uppercase tracking-wider ${countdownText === 'Lewat Deadline' ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400 animate-pulse'}`}>{countdownText}</div>
                                            <div className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mt-1">Sisa Waktu Pelaporan</div>
                                        </div>
                                    </div>

                                    {/* CARD 3: STATUS LAPORAN */}
                                    <div className="bg-gradient-to-br from-purple-50 to-purple-100/50 dark:from-purple-950/20 dark:to-gray-800 border border-purple-100 dark:border-purple-800/60 p-4 rounded-2xl shadow-sm flex items-center gap-4">
                                        <div className="p-3 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-xl">
                                            <i className="ph-fill ph-seal-check text-2xl"></i>
                                        </div>
                                        <div className="w-full">
                                            <div className="text-[10px] uppercase font-black tracking-widest text-purple-500 mb-0.5">Status Laporan</div>
                                            {isStaff ? (
                                                <span className={`inline-block px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getStatusBadgeClass(reportStatus)}`}>
                                                    {reportStatus}
                                                </span>
                                            ) : (
                                                <div className="flex flex-wrap gap-1.5 mt-0.5">
                                                    <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 text-[9px] font-bold text-gray-600 dark:text-gray-400 rounded">
                                                        Draft: {inputItems.filter(item => getReportStatus(item.p, filterTanggalKerja) === 'Draft').length}
                                                    </span>
                                                    <span className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 text-[9px] font-bold text-emerald-600 dark:text-emerald-400 rounded">
                                                        Sub: {inputItems.filter(item => getReportStatus(item.p, filterTanggalKerja) === 'Submitted').length}
                                                    </span>
                                                    <span className="px-2 py-0.5 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 text-[9px] font-bold text-rose-600 dark:text-rose-400 rounded">
                                                        Lock: {inputItems.filter(item => getReportStatus(item.p, filterTanggalKerja) === 'Locked').length}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    {/* CARD 4: TOTAL KANDIDAT */}
                                    <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/20 dark:to-gray-800 border border-emerald-100 dark:border-emerald-800/60 p-4 rounded-2xl shadow-sm flex items-center gap-4">
                                        <div className="p-3 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-xl">
                                            <i className="ph-fill ph-users-three text-2xl"></i>
                                        </div>
                                        <div>
                                            <div className="text-[10px] uppercase font-black tracking-widest text-emerald-500 mb-0.5">Total Kandidat Hari Ini</div>
                                            <div className="font-black text-sm text-emerald-600 dark:text-emerald-400">{totalCandidates} Pelamar</div>
                                            <div className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mt-1">Status Diterima (Acc)</div>
                                        </div>
                                    </div>
                                </div>
                                {/* Desktop View */}
                        ...
                        <div className="p-4 sm:p-6 lg:p-8 bg-gray-50/30 dark:bg-gray-900/20">
                            ...
                            {isLoading ? <div className="flex flex-col items-center justify-center py-20 text-indigo-500"><i className="ph-bold ph-spinner ph-spin text-5xl mb-3 drop-shadow-md"></i></div> : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div className="bg-gradient-to-b from-emerald-50/80 to-white dark:from-emerald-950/20 dark:to-gray-800 rounded-3xl p-5 border border-emerald-100 dark:border-emerald-800/40 shadow-sm flex flex-col justify-between">
                                        <div>
                                            <h4 className="font-black text-emerald-700 dark:text-emerald-400 mb-5 flex items-center justify-between border-b border-emerald-200/60 dark:border-emerald-800/60 pb-4"><span className="flex items-center text-base"><i className="ph-fill ph-check-circle mr-2 text-xl drop-shadow-sm"></i> Target Tercapai</span><span className="bg-emerald-500 text-white dark:bg-emerald-600 px-2.5 py-0.5 rounded-md text-[10px] font-black shadow-sm">{listSesuaiTarget.length} Orang</span></h4>
                                            <div className="space-y-4 flex-1">{paginatedSesuaiTarget.map(renderHarianCard)}{listSesuaiTarget.length === 0 && <div className="text-xs font-bold text-gray-400 text-center py-10 opacity-50"><i className="ph-fill ph-empty text-3xl mb-2 block"></i>Nihil</div>}</div>
                                        </div>
                                        {renderPagination(pageSesuaiTarget, listSesuaiTarget.length, setPageSesuaiTarget)}
                                    </div>
                                    <div className="bg-gradient-to-b from-rose-50/80 to-white dark:from-rose-950/20 dark:to-gray-800 rounded-3xl p-5 border border-rose-100 dark:border-rose-800/40 shadow-sm flex flex-col justify-between">
                                        <div>
                                            <h4 className="font-black text-rose-700 dark:text-rose-400 mb-5 flex items-center justify-between border-b border-rose-200/60 dark:border-rose-800/60 pb-4"><span className="flex items-center text-base"><i className="ph-fill ph-warning-circle mr-2 text-xl drop-shadow-sm"></i> Kena Denda</span><span className="bg-rose-500 text-white dark:bg-rose-600 px-2.5 py-0.5 rounded-md text-[10px] font-black shadow-sm">{listKurangTarget.length} Orang</span></h4>
                                            <div className="space-y-4 flex-1">{paginatedKurangTarget.map(renderHarianCard)}{listKurangTarget.length === 0 && <div className="text-xs font-bold text-gray-400 text-center py-10 opacity-50"><i className="ph-fill ph-empty text-3xl mb-2 block"></i>Nihil</div>}</div>
                                        </div>
                                        {renderPagination(pageKurangTarget, listKurangTarget.length, setPageKurangTarget)}
                                    </div>
                                    <div className="bg-gradient-to-b from-amber-50/80 to-white dark:from-amber-950/20 dark:to-gray-800 rounded-3xl p-5 border border-amber-100 dark:border-amber-800/40 shadow-sm flex flex-col justify-between">
                                        <div>
                                            <h4 className="font-black text-amber-700 dark:text-amber-400 mb-5 flex items-center justify-between border-b border-amber-200/60 dark:border-emerald-800/60 pb-4"><span className="flex items-center text-base"><i className="ph-fill ph-hand-palm mr-2 text-xl drop-shadow-sm"></i> Izin / Libur</span><span className="bg-amber-500 text-white dark:bg-amber-600 px-2.5 py-0.5 rounded-md text-[10px] font-black shadow-sm">{listIzin.length} Orang</span></h4>
                                            <div className="space-y-4 flex-1">{paginatedIzin.map(renderHarianCard)}{listIzin.length === 0 && <div className="text-xs font-bold text-gray-400 text-center py-10 opacity-50"><i className="ph-fill ph-empty text-3xl mb-2 block"></i>Nihil</div>}</div>
                                        </div>
                                        {renderPagination(pageIzin, listIzin.length, setPageIzin)}
                                    </div>
                                    <div className="bg-gradient-to-b from-gray-50/80 to-white dark:from-gray-900/30 dark:to-gray-800 rounded-3xl p-5 border border-gray-200 dark:border-gray-700/60 shadow-sm flex flex-col justify-between">
                                        <div>
                                            <h4 className="font-black text-gray-700 dark:text-gray-300 mb-5 flex items-center justify-between border-b border-gray-200 dark:border-gray-700/80 pb-4"><span className="flex items-center text-base"><i className="ph-fill ph-prohibit mr-2 text-xl drop-shadow-sm"></i> Belum Lapor</span><span className="bg-gray-500 text-white dark:bg-gray-600 px-2.5 py-0.5 rounded-md text-[10px] font-black shadow-sm">{listBelumLapor.length} Orang</span></h4>
                                            <div className="space-y-4 flex-1">{paginatedBelumLapor.map(renderHarianCard)}{listBelumLapor.length === 0 && <div className="text-xs font-bold text-gray-400 text-center py-10 opacity-50"><i className="ph-fill ph-empty text-3xl mb-2 block"></i>Nihil</div>}</div>
                                        </div>
                                        {renderPagination(pageBelumLapor, listBelumLapor.length, setPageBelumLapor)}
                                    </div>
                                </div>
                            )}
                        </div>
                                        ...
                                        {isLoading ? [...Array(5)].map((_,i)=><tr key={i}><td colSpan="11" className="px-5 py-6"><div className="h-6 bg-gray-200 dark:bg-gray-700 animate-pulse rounded-md w-full"></div></td></tr>) : 
                                         ...
                                         displayWeekly.map((s, i) => (
                                            <tr key={i} className="hover:bg-purple-50/30 dark:hover:bg-purple-900/10 transition-colors">
                                                <td className="px-5 py-4">
                                                     <div>
                                                         <div className="font-black text-sm text-gray-900 dark:text-white">{s.name || s.username}</div>
                                                         <div className="text-[10px] text-gray-400 font-mono mt-0.5">@{s.username}</div>
                                                     </div>
                                                 </td>
                                ...
                                <table className="w-full text-left whitespace-nowrap min-w-[800px]">
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                                        {perfData.filter(p => p.tanggalKerja.startsWith(riwayatBulan) && (riwayatUsername ? p.username === riwayatUsername : true) && (!isPrivileged ? p.username === authUser.username : true)).sort((a,b) => new Date(b.tanggalKerja).getTime() - new Date(a.tanggalKerja).getTime()).map((p, i) => {
                                            ...
                                            const statusVal = getReportStatus(p, p.tanggalKerja);
                                            const evalP = evaluateH1(p.tanggalKerja, p.tanggalLapor, auto.totalHarian, p.postingan, statusVal, p.isIzin || p.status === 'Izin' || p.isIzin === 'true');
                                            return (
                                                <tr key={i} className="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors">
                                                    <td className="px-5 py-4">
                                                         <div className="flex items-center gap-2">
                                                             <i className="ph-fill ph-user-circle text-gray-400 text-lg shrink-0"></i>
                                                             <div>
                                                                 <div className="font-black text-sm text-gray-900 dark:text-white">{users.find((u: any) => u.username === p.username)?.name || p.username}</div>
                                                                 <div className="text-[10px] text-gray-400 font-mono mt-0.5">@{p.username}</div>
                                                             </div>
                                                         </div>
                                                     </td>
                                                    <td className="px-5 py-4 text-center">
                                        ...
                                        <div key={i} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm">
                                            <div className="flex justify-between items-start mb-3">
                                                <div className="font-black text-sm text-gray-900 dark:text-white flex items-center gap-1.5">
                                                     <i className="ph-fill ph-user-circle text-gray-400 text-lg shrink-0"></i>
                                                     <div>
                                                         <div className="font-black text-sm">{users.find((u: any) => u.username === p.username)?.name || p.username}</div>
                                                         <div className="text-[10px] text-gray-400 font-mono mt-0.5">@{p.username}</div>
                                                     </div>
                                                 </div>
                                                <div className="text-right">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col relative z-10 animate-in zoom-in-95 duration-200 border-t-[6px] border-t-indigo-600">
                        ...
                        <form onSubmit={handleSavePerf} className="p-4 sm:p-6 lg:p-8 overflow-y-auto max-h-[85vh] custom-scrollbar bg-gray-50/30 dark:bg-gray-900/30">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                                {/* Tipe Laporan Section */}
                                <div className="md:col-span-2 bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <h4 className="font-black text-xs text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 pb-2 flex items-center mb-4"><i className="ph-bold ph-calendar-blank mr-2 text-indigo-500"></i> Opsi Laporan & Kehadiran</h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {isPrivileged ? (
                                            <>
                                                <button 
                                                    type="button" 
                                                    onClick={() => setFormData({ ...formData, isIzin: false })}
                                                    className={`p-4 rounded-xl font-bold text-xs border transition-all flex flex-col items-center justify-center gap-2 ${!formData.isIzin ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-100'}`}
                                                >
                                                    <i className="ph-bold ph-chart-line text-lg"></i>
                                                    <span>Laporan Kinerja Harian (Normal)</span>
                                                </button>
                                                <button 
                                                    type="button" 
                                                    onClick={() => setFormData({ ...formData, isIzin: true, postingan: '0', kunjungan: '0', pelamar: '0', pengujian: '0' })}
                                                    className={`p-4 rounded-xl font-bold text-xs border transition-all flex flex-col items-center justify-center gap-2 ${formData.isIzin ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:bg-gray-100'}`}
                                                >
                                                    <i className="ph-bold ph-hand-palm text-lg"></i>
                                                    <span>Pengajuan Izin / Libur (Bebas Denda)</span>
                                                </button>
                                            </>
                                        ) : (
                                            <div className="sm:col-span-2 bg-amber-50 dark:bg-amber-950/20 text-amber-800 dark:text-amber-400 p-4 rounded-xl border border-amber-200 dark:border-amber-800/30 font-bold text-xs flex items-center gap-3">
                                                <i className="ph-bold ph-info text-lg shrink-0"></i>
                                                <span>Sebagai Staff, Anda hanya diperbolehkan mengajukan Izin / Cuti. Laporan kinerja harian di-input oleh Admin / Superadmin.</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                                    {formData.isIzin ? (
                                        <>
                                            <h4 className="font-black text-xs text-amber-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 pb-2 flex items-center mb-4"><i className="ph-bold ph-warning-circle mr-2"></i> Keterangan Izin</h4>
                                            <div>
                                                <label className="block text-[10px] font-black text-gray-500 mb-1.5 uppercase tracking-widest ml-1">Alasan / Keterangan Izin</label>
                                                <textarea 
                                                    required 
                                                    rows={3}
                                                    placeholder="Contoh: Sakit dengan surat dokter, Cuti Tahunan, Keperluan Keluarga Mendadak, dll." 
                                                    className={`${InputClass} resize-none`} 
                                                    value={formData.keteranganIzin || ''} 
                                                    onChange={e=>setFormData({...formData, keteranganIzin: e.target.value})} 
                                                />
                                            </div>
                                        </>
                                    ) : (
